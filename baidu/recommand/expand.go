// this package is use to get baidu mobile recommend words
package recommand
...
func Expand(word string) (keywords []string, err error) {
	...
	for _, l := range lists {
		...
		ups := l.Get("up").Array()
		for _, up := range ups {
			keywords = append(keywords, up.String())
		}
		downs := l.Get("down").Array()
		for _, d := range downs {
			keywords = append(keywords, d.String())
		}
	}
